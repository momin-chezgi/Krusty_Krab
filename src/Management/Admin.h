...
class AdminOfSystem{
    ...
    Restaurateur *accessRestaurant(string restauraurateurID);   // Under-destruction :)
...
public:
    ...
    // restaurateur options:
        ...
        string getRestaurantName(ManagerID_tp restaurantID) const;
        string getRestaurantAddress(ManagerID_tp restaurantID) const;
        bool getRestaurantStatus(ManagerID_tp restaurantID) const;
        size_t getRestaurantPreparationMinutes(ManagerID_tp restaurantID) const;
        string getRestaurantPhone(ManagerID_tp restaurantID) const;
        string getRestaurantBio(ManagerID_tp restaurantID) const;
        ...
        // menu:
        string getMenuID(ManagerID_tp restaurateurID) const;
        ...
        bool removeItemFromMenu(ManagerID_tp restaurateurID, string itemID);
        // 'removeItemFromMenu' returns true if the item was in the menu and has been deleted
        bool replaceItemInMenu(ManagerID_tp restaurateurID, string previousItemID, const MenuItem& replacedItem);
        ...
        // order:
        bool addItemToOrder(ManagerID_tp restaurateurID, string orderID, const MenuItem& item, double quantity = 1);
        bool removeItemFromOrder(ManagerID_tp restaurateurID, string orderID, string itemID);   


        // queue:
        vector<string> getOrderIDs(ManagerID_tp restaurateurID) const;

        bool addOrderToQueue(ManagerID_tp restaurateurID, string orderID);
        // addOrderToQueue returns true if the order hasn't been in the orders(distinguished by id) and carefully added
        bool removeOrderFromQueue(ManagerID_tp restaurateurID, string orderID);
        // removeOrderFromQueue returns true if the order has been in the orders(distinguished by id) and carefully removed
        bool replaceOrderInQueue(ManagerID_tp restaurateurID, string previousOrderID, string newOrderID);
