...
// restaurateur options:
   ...
   string AdminOfSystem::getRestaurantName(ManagerID_tp restaurantID) const
   {
      RestaurantStorage storage;
      return storage.getName(restaurantID);
   }

   string AdminOfSystem::getRestaurantAddress(ManagerID_tp restaurantID) const
   {
      RestaurantStorage storage;
      return storage.getAddress(restaurantID);
   }

   bool AdminOfSystem::getRestaurantStatus(ManagerID_tp restaurantID) const
   {
      RestaurantStorage storage;
      return storage.getStatus(restaurantID);
   }

   size_t AdminOfSystem::getRestaurantPreparationMinutes(ManagerID_tp restaurantID) const
   {
      RestaurantStorage storage;
      return storage.getPreparationMinutes(restaurantID);
   }

   string AdminOfSystem::getRestaurantPhone(ManagerID_tp restaurantID) const
   {
      RestaurantStorage storage;
      return storage.getPhoneNumber(restaurantID);
   }

   string AdminOfSystem::getRestaurantBio(ManagerID_tp restaurantID) const
   {
      RestaurantStorage storage;
   ...
   string AdminOfSystem::getMenuID(ManagerID_tp restaurateurID) const
   {
      RestaurantStorage storage;
      RestID_tp restaurantID = getRestaurantID(restaurateurID);
   ...
   bool AdminOfSystem::addItemToMenu(ManagerID_tp restaurateurID, const MenuItem& item)
   {
      ...
      RestaurantStorage storage;
      RestID_tp restaurantID = getRestaurantID(restaurateurID);
      string menuID = storage.getMenuID(restaurantID);
      MenuStorage mStorage;
      return mStorage.addItem(menuID, item);
   }

   bool AdminOfSystem::removeItemFromMenu(ManagerID_tp restaurateurID, string itemID)
   {
      if(!hasRestaurateur(restaurateurID)) return false;
      RestaurantStorage storage;
      RestID_tp restaurantID = getRestaurantID(restaurateurID);
   ...
   bool AdminOfSystem::replaceItemInMenu(ManagerID_tp restaurateurID, string previousItemID, const MenuItem& replacedItem)
   {      
      if(!hasRestaurateur(restaurateurID)) return false;
      RestaurantStorage storage;
      RestID_tp restaurantID = getRestaurantID(restaurateurID);
   ...
   bool AdminOfSystem::addItemToOrder(ManagerID_tp restaurateurID, string orderID, const MenuItem& item, double quantity)
   {      
      if(!hasRestaurateur(restaurateurID)) return false;
   ...
   bool AdminOfSystem::removeItemFromOrder(ManagerID_tp restaurateurID, string orderID, string itemID)
   {      
      if(!hasRestaurateur(restaurateurID)) return false;
      OrderStorage storage;
   ...
   vector<string> AdminOfSystem::getOrderIDs(ManagerID_tp restaurateurID) const
   {      
   ...
   bool AdminOfSystem::addOrderToQueue(ManagerID_tp restaurateurID, string orderID)
   {      
      if(!hasRestaurateur(restaurateurID)) return false;
      RestaurantStorage storage;
      RestID_tp restaurantID = getRestaurantID(restaurateurID);
   ...
   bool AdminOfSystem::removeOrderFromQueue(ManagerID_tp restaurateurID, string orderID)
   {      
      if(!hasRestaurateur(restaurateurID)) return false;
      RestaurantStorage storage;
      RestID_tp restaurantID = getRestaurantID(restaurateurID);
   ...
   bool AdminOfSystem::replaceOrderInQueue(ManagerID_tp restaurateurID, string previousOrderID, string newOrderID)
   {      
      if(!hasRestaurateur(restaurateurID)) return false;
      RestaurantStorage storage;
      RestID_tp restaurantID = getRestaurantID(restaurateurID);
